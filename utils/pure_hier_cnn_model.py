...
class SentenceLSTM(nn.Module):
    ...
    def forward(self, avg_cat_features,  state):
        ...
        topic = self.relu( self.fc1( output ))
        p_stop = self.W_stop(output)
        
        return topic, p_stop, state_t
...
class WordLSTM(nn.Module):
    def __init__(self,
        ...
        self.affine_x = nn.Linear(hidden_size+embed_size, embed_size)
        self.__init_weights()
        self.n_max = n_max
    ...
    def __init_weights(self):
        ...
        self.affine_x.weight.data.uniform_(-0.1, 0.1)
        self.affine_x.bias.data.fill_(0)



    def forward(self, topic_vec, captions, states):
        """
        state initialized to None and embedding is cat[topic, caption_embed]
        """
        embeddings = self.embed(captions).unsqueeze(1)
        embeddings = torch.cat((topic_vec, embeddings), 2)
        inputx = self.affine_x(embeddings)
        hidden, states_t = self.lstm(inputx, states)
        outputs = self.linear(hidden[:, -1, :])
        return outputs, states_t



    def sample(self, topic, start_tokens):
        sampled_ids = np.zeros((np.shape(topic)[0], self.n_max))
        ...
        for i in range(1, self.n_max):
            ...
            outputs, states_t = self.forward(topic, predicted, states)
            states = states_t
